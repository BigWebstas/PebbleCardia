...
| **Phone** | `src/pkjs/index.js` keeps a rolling log of HR/HRV samples (one per ~15 s) and rhythm episodes in `localStorage`. On launch it asks the watch to replay every stored episode (`DUMP`), so episodes the background worker recorded while the app was closed still reach the phone. |
| **Export** | The configuration page (Pebble app → Cardia → settings) builds a **Markdown report** with Mermaid charts (HR / RMSSD / pNN50 line charts, an episode Gantt + pie, tables) and offers Save / Share / Copy for the `.md` and the raw `.json`. Optionally set a **sync URL** and every sample + episode is `POST`ed there as JSON as it arrives (e.g. a Google Apps Script that appends to a Sheet). |
